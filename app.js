...
function generatePlans() {
  ...
  for (let rowInterval = 1; rowInterval <= Math.min(8, input.rows); rowInterval++) {
    for (let treeInterval = 1; treeInterval <= Math.min(20, input.treesPerRow); treeInterval++) {
      for (let offset = 0; offset < treeInterval; offset++) {
        const layout = buildLayout(input.rows, input.treesPerRow, rowInterval, treeInterval, offset);
...
function buildLayout(rows, treesPerRow, rowInterval, treeInterval, offset) {
  ...
  for (let r = 1; r <= rows; r++) {
    ...
    const rowOffset = offset > 0 ? ((r - 1) * offset) % treeInterval : 0;
...
function renderOptions(plans, input, targetDispensers) {
  ...
  plans.forEach((plan, index) => {
    ...
    card.innerHTML = `
      ...
      <p class="muted">
        Difference from target: ${plan.count - targetDispensers > 0 ? "+" : ""}${plan.count - targetDispensers} dispensers
      </p>
...
function describePattern(plan) {
  const rowText = plan.rowInterval === 1
    ? "every row"
    : `every ${ordinal(plan.rowInterval)} row`;

  const treeText = plan.treeInterval === 1
    ? "every tree"
    : `every ${ordinal(plan.treeInterval)} tree`;

  const offsetText = plan.offset > 0
