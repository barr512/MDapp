...
def find_inverse_hs_v3(df):
    ...
    # NEW: Pattern points for charting
    pattern_points = {
        "left_shoulder": {"time": recent.index[ls_idx].strftime("%Y-%m-%d"), "value": round(ls, 2)},
        "head": {"time": recent.index[h_idx].strftime("%Y-%m-%d"), "value": round(head, 2)},
        "right_shoulder": {"time": recent.index[rs_idx].strftime("%Y-%m-%d"), "value": round(rs, 2)},
        "neckline": round(neckline, 2),
        "neckline_start": recent.index[ls_idx].strftime("%Y-%m-%d"),
        "neckline_end": recent.index[rs_idx].strftime("%Y-%m-%d")
    }
    
    # NEW: Last 180 days of OHLC for chart
    ohlc = recent.tail(180).reset_index()
    ohlc['time'] = ohlc['timestamp'].dt.strftime("%Y-%m-%d")
    chart_data = ohlc[['time', 'open', 'high', 'low', 'close']].to_dict('records')
    
    return {
        "ticker": None, "price": round(current_price, 2), "neckline": round(neckline, 2), 
        "status": status, "breakout_date": recent.index[-1].strftime("%Y-%m-%d") if status == "broken_out" else None, 
        "target": round(target, 2), "proj_gain": round((target / current_price - 1) * 100, 1), 
        "vol_spike": vol_spike, "rsi": rsi_val, "risk_reward": risk_reward, 
        "trend": "strong" if trend_ok else "weak", "trend_detail": trend_reason, "stop": round(stop, 2),
        "pattern_points": pattern_points, "ohlc": chart_data # NEW FIELDS
    }
...
def scan_and_backtest():
    ...
    for i, ticker in enumerate(TICKERS):
        ...
        if pattern:
            ...
            results.append({k:v for k,v in pattern.items() if k!= 'ohlc'}) # data.json without huge OHLC
            chart_data[ticker] = {"ohlc": pattern["ohlc"], "pattern_points": pattern["pattern_points"]} # separate file
    ...
    with open("chart_data.json", "w") as f: json.dump(chart_data, f) # NEW: OHLC + pattern points
    with open("backtest.json", "w") as f: json.dump(backtests, f, indent=2)
    print(f"Done. {len(results)} current patterns. Charts saved.")
